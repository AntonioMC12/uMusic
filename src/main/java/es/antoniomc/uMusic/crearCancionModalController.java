...
public class crearCancionModalController {
	...
	public void initialize() {
		...
		slDuracion.valueProperty().addListener(
	...
	void crearCancion(ActionEvent event) {
    	if(checkFields()) {
    		...
    		try {
				cancionDAO.guardar(new Cancion(-1L, this.txtNombre.getText(), (float) this.slDuracion.getValue()-1, this.cbDisco.getValue(), this.cbGenero.getValue(), 0));
    ...
    private boolean checkFields() {
    	return (!this.txtNombre.getText().trim().isEmpty() && !this.cbDisco.getSelectionModel().isEmpty() && !this.cbGenero.getSelectionModel().isEmpty());
    }
